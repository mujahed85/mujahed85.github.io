...
const devopsSlugs = blogsData.filter(post => post.tags && post.tags.some(tag => tag.toLowerCase().includes('devops'))).map(post => post.slug);
...
export default async function BlogPostPage({ params }) {
  const { slug } = params;
  if (!devopsSlugs.includes(slug)) notFound();
  ...
  return (
    <div className="container my-5">
      <a href="/blogs/devops" className="btn btn-link mb-3">&larr; Back to DevOps Blogs</a>
