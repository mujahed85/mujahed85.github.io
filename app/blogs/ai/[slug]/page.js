...
const aiSlugs = blogsData
  .filter(
    (post) =>
      post.tags && post.tags.some((tag) => tag.toLowerCase().includes("ai"))
  )
  .map((post) => post.slug);
...
export async function generateMetadata({ params }) {
  ...
  const mdPath = findMarkdownPath("ai", slug);
  try {
...
export default async function BlogPostPage({ params }) {
  ...
  const mdPathForEdit = findMarkdownPath("ai", slug);
      ...
      <section className="container spacer">
        <div className="row">
          ...
          <div className="col-md-8 col-lg-9">
            <a href="/blogs/ai" className="btn btn-link mb-3">
              &larr; Back to AI Blogs
            </a>
            <a
              href={githubEditUrl}
              target="_blank"
